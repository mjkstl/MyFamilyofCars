...
export default function CollectionPhotoGallery({
  ...
  if (orderedPhotos.length === 0) {
    return <View style={[styles.fallback, { height }]}><Text style={styles.fallbackText}>No photo yet</Text></View>;
  }

  return (
    <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} style={{ height }} accessibilityLabel={`${orderedPhotos.length} car photo${orderedPhotos.length === 1 ? '' : 's'}`}>
      {orderedPhotos.map((photo) => (
        <View key={photo.id} style={{ width: '100%', height }}>
